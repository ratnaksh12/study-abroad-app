...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function generateUniversities(userData, activeFilters = null) {
        ...
        try {
            ...
            // Fetch from API
            const response = await fetch(`http://localhost:3001/api/universities?${params.toString()}`);
            ...
            if (!data.success || data.universities.length === 0) {
                ...
                const allResponse = await fetch(`http://localhost:3001/api/universities`);
        ...
        if (currentFilter === 'shortlisted' && userData.shortlistedUniversities.length > 0) {
            ...
            try {
                ...
                const promises = userData.shortlistedUniversities.map(id => {
                    ...
                    return fetch(`http://localhost:3001/api/universities/${id}`)
                        .then(res => res.json())
                        .then(data => {
